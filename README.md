...
零依赖静态网页：每个交易日收盘后运行一次采集脚本，自动生成一张包含「核心裁决 → 盘面全景与风格坐标 → 近五日市场 → 资金流向 → 涨停梯队与情绪结构 → 全球市场联动 → 市场广度与两融 → 蓄力观察池 → 来源与风险」的复盘决策卡，可打印导出 PDF。
...
| 数据项 | 来源 | 稳态请求数/日 |
|---|---|---|
| 7 指数日线（K线 + 多窗口收益） | 腾讯 fqkline | 7 |
| 沪市成交额 | [中证指数官网](https://www.csindex.com.cn) index-perf | 1 |
| 深市成交额 | [深交所官网](https://www.szse.cn) 每日行情 | 1（历史走缓存） |
| 涨停 / 跌停 / 炸板池 | 东方财富 push2ex | 3（历史走缓存） |
| 涨跌家数（广度）、新高新低 | 新浪 A 股全列表 | ~58（100/页分页） |
| 行业主力资金流（当日/5日/10日） | 东方财富板块资金排行 | ~6（分页 100/页；实时集群失败自动切延时集群 push2delay） |
| 两融余额（T+1） | [东方财富数据中心](https://datacenter-web.eastmoney.com) `RPTA_RZRQ_LSHJ` | 1 |
| 全球指数（10 个美 / 欧 / 亚太 / 汇率） | push2delay `ulist.np/get`（批量 secid） | 1 |
| 费半 + 黄金 / 白银 / 原油 / 伦铜 / 离岸人民币 | 新浪 `hq.sinajs.cn`（批量 list） | 1 |
| 恒生科技 | 腾讯 qt.gtimg.cn | 1 |

东方财富合计 **约 12 次/日**（naive 全量拉取约 97 次/日，实测数分钟内即触发 IP 级限流）。资金流的当日/5日/10日直接来自东财排行接口的官方聚合字段（f62/f164/f174），无需历史回填；当实时集群（push2）被限流时自动切换延时集群（push2delay），收盘后两者数值完全一致。两融与全球行情都是**软依赖**：失败时只让对应章节为空并在风险栏标注，不影响其余数据。
...
- `index_panel[]`：7 指数 × {name、close、pct、ret5/ret20/ret60}，行情源腾讯 fqkline
- `limit_ladder`：`ladder[]`（按连板降序）、`distribution`（板层→家数）、`metrics`（涨停/跌停/炸板/封板率/晋级率/最高连板/2 板及以上）、`notes[]`（口径说明）
- `margin`：`balance`（亿元）、`change`、`financing`/`securities_loan`/`financing_net_buy`、`pct_of_float`（占流通市值%）、`as_of`（T+1 披露日）
- `global_markets[]`：`name/category/close/pct/as_of/lagged`；`global_as_of`、`global_us_session`（"closed" 或 "intraday"）
...
- **涨停天梯**：按连板数降序、同板按封单力度降序；封板率 = 收盘涨停 /（收盘涨停+炸板）；晋级率 = 上一交易日涨停股中今日仍涨停的占比（依赖本地缓存的每日涨停代码列表）
- **风格坐标**：20 日 `中证1000-沪深300` 与 `中证红利-沪深300` 差值，按 ±1.0pct 阈值归类（仅规则化描述，不做叙事判断）
