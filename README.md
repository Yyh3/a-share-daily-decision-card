...
| 行业主力资金流（当日/5日/20日） | 东方财富板块资金排行 | **1**（5日/20日由缓存逐日累加） |

东方财富合计 **3~4 次/日**（naive 全量拉取约 97 次/日，实测数分钟内即触发 IP 级限流）。首次运行会一次性回填约 90 个板块的资金流历史（~90 次请求、限速 0.35s/次），此后不再回填。

**缓存文件**（`data/cache/`，可随时删除重建）：

- `daily_stats.json` — 每日涨跌停数、深市成交额
- `sector_flow_history.json` — 每板块逐日主力净流入
...
- `flows[]`：sector、today/day5/day20 主力净流入（亿元，JSON number）、change_pct、limit_up
...
- **资金持续性**：5 日与 20 日资金正负组合 → 持续流入/拐点回流/拐点撤退/持续流出
...
4. 免费接口无 SLA；东财接口被限流时资金流章节自动降级为空并在页面标注，其余数据不受影响；
